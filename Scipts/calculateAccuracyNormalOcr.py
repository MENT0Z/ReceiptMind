...
GT_DIR = "Gt/boxes"
OCR_JSON_PATH = "all_ocr_results.json"
MISMATCH_FILE = "text_mismatches.json"
...
def load_pred_boxes(ocr_json):
    return [{"box": item["box"], "text": item["text"]} for item in ocr_json["content"]]
...
def recognition_accuracy(gt_boxes, pred_boxes, image_name, mismatches, iou_thresh=0.5):
    ...
    return {
        "avg_cer": avg_cer,
        "recognition_accuracy": 1 - avg_cer,
...
if __name__ == "__main__":
    ...
    rec_accuracies, rec_cers = [], []
    mismatches = []

    for image_name, ocr_data in all_results.items():
        ...
        pred_boxes = load_pred_boxes(ocr_data)

        det = detection_metrics(gt_boxes, pred_boxes)
        rec = recognition_accuracy(gt_boxes, pred_boxes, image_name, mismatches)

        det_precisions.append(det["precision"])
        det_recalls.append(det["recall"])
        det_f1s.append(det["f1"])

        rec_accuracies.append(rec["recognition_accuracy"])
        rec_cers.append(rec["avg_cer"])
    ...
    print("\n===== OVERALL AVERAGES =====")
    print("Detection Precision:", sum(det_precisions) / len(det_precisions))
    print("Detection Recall   :", sum(det_recalls) / len(det_recalls))
    print("Detection F1       :", sum(det_f1s) / len(det_f1s))
    print("Recognition Acc    :", sum(rec_accuracies) / len(rec_accuracies))
